...
class Strategy:
    ...
    @abstractmethod
    def _get_entity_picks(self, query_embs: QueryEmbeddings, n: int, questionID: int) -> Union[tuple[Article, ...], list[tuple[Article, ...]]]:
        """
    ...
    @abstractmethod
    def _get_relation_picks(self, query_embs: QueryEmbeddings, n: int, questionID: int) -> Union[tuple[Article, ...], list[tuple[Article, ...]]]:
        """
        Retrieve the n documents closest to query_embs for entity-focused retrieval.
        """
        pass
...
class StandardStrategy(Strategy):
    ...
    def _get_entity_picks(self, query_embs: QueryEmbeddings, n: int, question_id: int) -> tuple[Article, ...]:
        return tuple(doc for (_, doc) in self.db.entity_search(question_id, query_embs.embeddings, n))

    def _get_relation_picks(self, query_embs: QueryEmbeddings, n: int, question_id: int) -> tuple[Article, ...]:
        return tuple(doc for (_, doc) in self.db.edge_search(question_id, query_embs.embeddings, n))
...
class MultiHeadStrategy(Strategy):
    ...
    def _entity_search(self, emb: FullEmbeddings, n: int,question_id: int) -> list[list[tuple[float, Article]]]:
        return self.db.entity_attention_search(
                                                emb = emb,
    ...
    def _relation_search(self, emb: FullEmbeddings, n: int,question_id: int) -> list[list[tuple[float, Article]]]:
        return self.db.edge_attention_search(
                                                emb = emb,
                                                layer_idx= self.layer, 
                                                n = n, 
                                                query_id=question_id
    ...
    def _multi_vote_entity(self, emb: FullEmbeddings, n: int,question_id: int) -> list[tuple[float, Article]]:
        votes: dict[Article, float] = {}
        ranking = self._entity_search(emb, n,question_id)
        head_scales: list[float] = self._get_head_scales()
    ...
    def _multi_vote_edge(self, emb: FullEmbeddings, n: int,question_id: int) -> list[tuple[float, Article]]:
        votes: dict[Article, float] = {}
        ranking = self._relation_search(emb, n,question_id)
        head_scales: list[float] = self._get_head_scales()
    ...
    def _get_entity_picks(self, query_embs: QueryEmbeddings, n: int,question_id : int) -> tuple[Article, ...]:
        ...
        :type: tuple[Article, ...]
        """
        return tuple(doc for (votes, doc) in self._multi_vote_entity(query_embs.embeddings, n,question_id))
    ...
    def _get_relation_picks(self, query_embs: QueryEmbeddings, n: int,question_id : int) -> tuple[Article, ...]:
        ...
        :type: tuple[Article, ...]
        """
        return tuple(doc for (votes, doc) in self._multi_vote_edge(query_embs.embeddings, n,question_id))
